  ...
  const PhaseContent = ({ index }: { index: number }) => (
    <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-center w-full">
      <div className="flex-1 w-full text-left">
        <h2 className="text-2xl md:text-3xl font-bold mb-4 md:mb-6 tracking-tight text-white">The {steps[index].title} Phase</h2>
    ...
    <div className="pt-20">
      <section className="py-12 md:py-24 bg-slate-950 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mb-12 md:mb-16">
            <h1 className="text-4xl md:text-7xl font-black mb-6 md:mb-8 tracking-tight text-white">Security Science.</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-16 md:mb-24">
            {steps.map((step, idx) => (
              <div 
                key={idx}
                className={`p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border transition-all duration-500 cursor-pointer transform active:scale-95 ${
                  activeStep === idx 
              ...
              >
                ...
                <p className={`text-xs md:text-sm leading-relaxed transition-colors duration-500 ${activeStep === idx ? 'text-indigo-100' : 'text-slate-500'}`}>
                  {step.desc}
                </p>
              </div>
            ))}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 lg:p-16 overflow-hidden relative">
             {/* Physical Track Viewport */}
      ...
      <section className="py-12 md:py-24 bg-slate-950">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-black mb-8 md:mb-12 text-center tracking-tight text-white">Implementation FAQ</h2>
            ...
            {[
              ...
              <div key={idx} className="bg-slate-900/50 border border-slate-800 rounded-xl md:rounded-2xl overflow-hidden">
                <button className="w-full px-6 py-4 md:px-8 md:py-6 text-left flex justify-between items-center hover:bg-slate-800/50 transition-colors">
                  <span className="font-bold text-sm md:text-lg text-white">{faq.q}</span>
                  <ChevronDown className="w-4 h-4 md:w-5 md:h-5 text-slate-500 shrink-0 ml-2" />
                </button>
                <div className="px-6 pb-4 md:px-8 md:pb-6 text-xs md:text-sm lg:text-base text-slate-400 leading-relaxed border-t border-slate-800/50 pt-3 md:pt-4 text-left">
                  {faq.a}
                </div>
              </div>
            ))}
